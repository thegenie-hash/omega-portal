...
function renderClientTable() {
  ...
  const rows = clients.map(c => `
    <tr>
      <td><strong>${esc(c.name)}</strong><br><span style="color:var(--faint);font-size:0.75rem">${esc(c.email)}</span></td>
      <td>${esc(c.firm || '—')}</td>
      <td>${c.account_size ? fmt(c.account_size) : '—'}</td>
      <td>${c.equity != null ? fmt(c.equity) : '—'}</td>
      <td><span class="status-pill status-${c.status || 'Pending'}">${esc(c.status || 'Pending')}</span></td>
      <td>
